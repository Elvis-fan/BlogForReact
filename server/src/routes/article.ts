...
export class Article {
    ...
    async getArticles({ params }) {
        ...
        const classes = await this.classesCollection.find({ pid: type }, { projection: { id: 1, _id: 0 } }).map(v => v.id).toArray()
        classes.push(type)
        return await this.collection.find({ class: { $in: classes } }, { skip: page * size, limit: Number(size), projection: { _id: 0, content: 0, images: 0 } }).toArray()
    }
